...
public sealed class AddressParser
{
    ...
    private static readonly Regex StreetNumberRegex = new(@"(?<number>\d{1,5}[\p{L}]?)\b", RegexOptions.Compiled);
    ...
    public PlaceInfoItem Parse(string? ocrText)
    {
        ...
        if (postalIndex >= 0 && postalLine != null)
        {
            ...
            var city = Regex.Replace(postalLine.Replace(match.Value, string.Empty), @"\s+", " ").Trim();
    ...
    public string NormalizeReadText(string? text)
    {
        ...
        var alphanumericOnly = NonAlphanumericRegex.Replace(text, " ");
        return Regex.Replace(alphanumericOnly, @"\s+", " ").Trim();
    }
    ...
    private static (string Name, string Number) SplitStreet(string? street)
    {
        ...
        var match = Regex.Match(normalized, @"^(?<name>.+?)\s+(?<number>\d{1,5}[\p{L}]?)\b", RegexOptions.IgnoreCase);
